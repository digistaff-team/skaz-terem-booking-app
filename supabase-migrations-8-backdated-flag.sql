-- ============================================================
-- Миграция 8: Явный признак «бронь внесена задним числом»
--
-- Нужен для отдельного учёта часов пост-учёта посещаемости
-- (миграция 6) в статистике — раньше отличить такую бронь от
-- обычной можно было только вручную сравнивая date и created_at.
--
-- is_backdated проставляется сервером в create_booking — TRUE,
-- если p_date на момент создания уже был в прошлом (та же ветка
-- условия, что разрешает бронирование задним числом админам).
-- Клиент этот флаг не передаёт и повлиять на него не может.
-- ============================================================

ALTER TABLE bookings ADD COLUMN IF NOT EXISTS is_backdated BOOLEAN NOT NULL DEFAULT FALSE;
...
BEGIN
  ...
  INSERT INTO bookings
    (room_id, room_name, date, start_time, end_time, title, description, user_name, user_id, status, is_backdated)
  VALUES
    -- p_date::DATE вставляется и в DATE-, и в TEXT-колонку (ISO-формат)
    (p_room_id, p_room_name, p_date::DATE, p_start_time, p_end_time,
     btrim(p_title), coalesce(p_description, ''), v_owner_name, v_owner_id, 'active', v_is_backdated)
