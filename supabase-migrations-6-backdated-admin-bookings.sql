-- ============================================================
-- Миграция 6: Бронирование задним числом для администраторов
--
-- Нужна для пост-учёта посещаемости Терема: администратор может
-- задним числом создать бронь на уже прошедшую дату (не глубже
-- 30 дней назад), чтобы зафиксировать факт использования помещения.
-- Списание с депозита часов и учёт в месячном лимите происходят
-- как для обычной брони — отдельной логики не требуется.
--
-- Ограничение на будущее (+365 дней) не меняется.
-- Сигнатура create_booking не меняется.
-- ============================================================
