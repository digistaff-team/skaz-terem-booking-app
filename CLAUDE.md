...
- `/admin/stats` — `AdminStats.tsx`: booking statistics for admins (client-side guard only — bookings are publicly readable anyway). Fetches all bookings, filters by period, and renders KPI tiles + pure-CSS bar charts (days *or* months/rooms/weekdays/start hours) + top-users list. Aggregation lives in `src/lib/stats.ts` (`computeStats`, unit-tested).
  - **Period** handling lives in `src/lib/statsPeriod.ts` — `Period`, `PERIODS`, `periodRange`, `isRangeInvalid`, `defaultCustomRange`, `shouldUseDayChart`, `DAY_CHART_MAX_DAYS`; all pure and unit-tested in `src/test/statsPeriod.test.ts`. Four presets plus `custom`, which reveals two native `<input type="date">` fields (shadcn `Input` + `Label`, matching `Admin.tsx`) prefilled with «1st of this month → today». An empty field means an open-ended bound; `from > to` replaces the charts with an error card. `periodRange`/`defaultCustomRange` take an optional `today` so the tests can pin a date — the page never passes it.
  - **Days vs months:** a range of `DAY_CHART_MAX_DAYS` (62) days or fewer renders the per-day chart (`byDay`) and hides the per-month one. Unset bounds are resolved from the min/max booking date first, so «30 дней» with a far-future booking doesn't silently become a half-year. `byDay` skips days with no bookings rather than drawing zero-height gaps, so it is a list of bars, not a continuous axis.
  - All charts are single-measure → one fill color: the `accent` token (~4:1 contrast vs card; `primary` fails 3:1) with direct value labels — no hover dependency on mobile. The day chart keeps that promise at scale by labelling every bar up to 15 and only the peak bar (`peakDayIndex`) beyond that — the same trick the start-hour chart uses with `peakHour` — because a «12,5 ч» label is ~34px and ten of them don't fit a phone card. Axis labels thin to ~10 via `showDayLabel`, which always labels the last bar (the end date the admin picked) and drops the regular label that would land within one step of it.
...
### Personal Hour History (моя статистика по месяцам)

`/account` also shows each resident **their own** booked hours month by month (`src/components/MyMonthlyStats.tsx`): a bar per month (last 12 months that have bookings — empty months are skipped, not drawn as gaps), tapping a bar opens that month's per-room breakdown. Unlike `PersonalLimit`, which only covers the current month, this block is the history — it is how a user checks a **past** month.

One query feeds it — `getBookings(userId)` under `["myBookingsHistory", userId]` (all of that user's rows, any status) — and everything else is computed client-side by `computePersonalMonthlyStats` (`src/lib/personalStats.ts`, unit-tested): cancelled bookings and other users' rows are dropped, months are returned **ascending** (chart order), rooms within a month descending by minutes. A `whole-house` booking is its own row and does **not** fan out across the individual rooms the way the shared pool does in `computeMonthUsage` — this counts the resident's own time, not room occupancy. Legacy rows with no `user_id` never appear. The selected month lives in `useState` and falls back to the newest month when the picked one disappears (e.g. its only booking was cancelled), so no `useEffect` sync is needed. `["myBookingsHistory"]` is invalidated after create/cancel alongside the other booking queries.
